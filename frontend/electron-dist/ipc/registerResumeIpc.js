...
export function registerResumeIpc() {
    ipcMain.handle('tailor:resume', async (_event, payload) => {
        const { resumeFile, job, jobInputType } = payload ?? {};
        if (!resumeFile || !job)
            throw new Error('Resume file and job input are required');
...
function inferFromInput(job, inputType) {
